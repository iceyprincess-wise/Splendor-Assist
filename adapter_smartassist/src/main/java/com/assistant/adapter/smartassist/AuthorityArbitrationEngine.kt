    ...
    fun arbitrate(
        ...
        return when(mode){
            ...
            1 -> ArbitrationResult(
                ...
                passY + ((forward * 48f) + (stability * 8f)).coerceIn(-120f,120f)
            )

            2 -> ArbitrationResult(
                predictiveX + ((shot * 50f) + (receiver * 64f)).coerceIn(-120f,120f),
                predictiveY + ((recovery * 60f) + (stability * 8f)).coerceIn(-120f,120f)
            )

            else -> ArbitrationResult(
                crossX + (receiver * 64f).coerceIn(-120f,120f),
                crossY + ((forward * 36f) + (stability * 8f)).coerceIn(-120f,120f)
            )
        }
    }
}
