...
class MagneticDashAnchor(
    ...
    fun processHighSpeedDribble(
        ...
        val dynamicIntervalNs = if (dragVelocity > OMEGA_TURNING_THRESHOLD) {
            16_666_666L // 60Hz turn priority update (16.6ms)
        } else {
            33_333_333L // 30Hz linear lock update (33.3ms)
        }.coerceAtLeast(MIN_PULSE_INTERVAL_NS)
        ...
        if (elapsedNs > dynamicIntervalNs) {
            ...
            val targetX = dashX + (cos(angle) * optimizedDistance).toFloat()
            val targetY = dashY + (sin(angle) * optimizedDistance).toFloat()
