...
/**
 * Single source of truth for the 23 admin-tunable runtime constants.
 *
...
object AdminConfigStore {
    ...
    data class Tunable(val key: String, val label: String, val def: Float)

    // ---- The 23 migrated constants (defaults = previous hard-coded values) ----
    val TUNABLES: List<Tunable> = listOf(
        // NetProbeEngine
        Tunable("net.probe.fast_ms",           "NetProbe dirty-link cadence (ms)",      2000f),
        Tunable("net.probe.calm_ms",           "NetProbe clean-link cadence (ms)",      5000f),
        Tunable("net.probe.timeout_ms",        "NetProbe TCP connect timeout (ms)",     1200f),
        Tunable("net.probe.alpha",             "NetProbe EWMA alpha",                   0.35f),
        // PacketLossProbeEngine
        Tunable("net.loss.round_ms",           "Loss probe round interval (ms)",        4000f),
        Tunable("net.loss.per_round",          "Loss probe queries per round",          4f),
        Tunable("net.loss.reply_timeout_ms",   "Loss probe reply timeout (ms)",         700f),
        Tunable("net.loss.alpha",              "Loss EWMA alpha",                       0.3f),
        // DnsWarmupEngine
        Tunable("net.dns.rewarm_ms",           "DNS re-warm interval (ms)",             90000f),
        // CongestionSentinelEngine
        Tunable("net.sentinel.poll_ms",        "Sentinel poll cadence (ms)",            2000f),
        Tunable("net.sentinel.rise_factor",    "Sentinel jitter rise factor",           1.5f),
        Tunable("net.sentinel.rise_fraction",  "Sentinel rise fraction of tolerance",   0.6f),
        // ActionWindowEngine
        Tunable("net.window.poll_ms",          "Action window refresh (ms)",            2000f),
        Tunable("net.window.hold_loss_pct",    "HOLD when loss above (%)",              10f),
        Tunable("net.window.go_loss_pct",      "GO requires loss below (%)",            2f),
        Tunable("net.window.hold_jitter_mult", "HOLD when jitter > tolerance x",        2f),
        // SpikeBurstEngine
        Tunable("net.spike.recovery_window_ms","Spike recovery watch window (ms)",      60000f),
        Tunable("net.spike.clean_samples",     "Clean samples to declare recovery",     2f),
        // RadioKeepAliveEngine
        Tunable("net.keepalive.floor_s",       "Keep-alive dirty-link floor (s)",       4f),
        // FramePacingEngine
        Tunable("lag.frame.alpha",             "Frame pacing EWMA alpha",               0.2f),
        Tunable("lag.frame.report_ms",         "Frame pacing report window (ms)",       20000f),
        Tunable("lag.frame.stall_ms",          "Hard stall threshold (ms)",             100f),
        // LoadShedGovernor
        Tunable("lag.shed.min_hold_ms",        "Load shed min hold (ms)",               8000f)
    )
